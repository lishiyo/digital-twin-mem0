...
class ChatGraphitiIngestion:
    ...
    def __init__(self, db_session: Session, graphiti_service: Optional[GraphitiService] = None):
    ...
    def process_message(self, message: ChatMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            # Initialize extraction results dictionary
            extraction_results = {
                "entities": [],
                "relationships": [],
                "traits": []
            ...
            # First, extract entities and relationships using entity_extractor
            entity_results = self.entity_extractor.process_document(message.content)
            extraction_results["entities"] = entity_results.get("entities", [])
            extraction_results["relationships"] = entity_results.get("relationships", [])
            
            # Next, use the TraitExtractionService for trait extraction
            if user and user.profile:
                try:
                    # Metadata for trait extraction
                    metadata = {
                        "message_id": message.id,
                    ...
                    # Extract traits using the service - returns a dict with success and traits (dict list)
                    # This will also update the UserProfile
                    trait_result = self.trait_service.extract_traits(
                        content=message.content,
                        source_type="chat",
                        user_id=message.user_id,
                        metadata=metadata,
                        # Let the service handle profile updates directly
                        update_profile=True
                    )
                    logger.info(f"process_message! Trait result from extract_traits: {trait_result}")
                    
                    if trait_result and trait_result.get("status") == "success":
                        # logger.info(f"Extracted {len(trait_result.get('traits', []))} traits using TraitExtractionService")
                        
                        # # Convert traits from service format to Graphiti format
                        # graphiti_traits = []
                        # for trait_dict in trait_result.get('traits', []):
                        #     # Apply confidence thresholds based on trait type
                        #     trait_type = trait_dict.get("trait_type", "").lower()
                        #     confidence = trait_dict.get("confidence", 0)
                            
                        #     # Skip traits that don't meet minimum confidence for their type
                        #     min_confidence = self.MIN_CONFIDENCE_TRAIT
                        #     if trait_type == "skill":
                        #         min_confidence = self.MIN_CONFIDENCE_TRAIT_SKILL
                        #     elif trait_type == "interest":
                        #         min_confidence = self.MIN_CONFIDENCE_TRAIT_INTEREST
                        #     elif trait_type == "preference":
                        #         min_confidence = self.MIN_CONFIDENCE_TRAIT_PREFERENCE
                        #     elif trait_type == "dislike":
                        #         min_confidence = self.MIN_CONFIDENCE_TRAIT_DISLIKE
                        #     elif trait_type == "attribute":
                        #         min_confidence = self.MIN_CONFIDENCE_TRAIT_ATTRIBUTE
                                
                        #     if confidence < min_confidence:
                        #         logger.debug(f"Skipping trait {trait_dict.get('name')} due to low confidence: {confidence}")
                        #         continue
                                
                        #     # Create Graphiti-compatible trait object
                        #     graphiti_trait = {
                        #         "name": trait_dict.get("name", ""),
                        #         "trait_type": trait_type,
                        #         "confidence": confidence,
                        #         "evidence": trait_dict.get("evidence", ""),
                        #         "strength": trait_dict.get("strength", 0.7),
                        #         "source": "chat",
                        #         "source_id": message.id
                        #     }
                        #     graphiti_traits.append(graphiti_trait)
                        
                        extraction_results["traits"] = trait_result.get("traits", [])
                    else:
                        logger.warning(f"Trait extraction failed or returned no traits: {trait_result}")
                except Exception as e:
                    logger.error(f"Error extracting traits using TraitExtractionService: {e}")
                    # Continue processing with just entities and relationships
            
            # Log extracted data for debugging
            logger.info(f"Extracted data: {len(extraction_results.get('entities', []))} entities, "
                       f"{len(extraction_results.get('relationships', []))} relationships, "
                       f"{len(extraction_results.get('traits', []))} traits")
            
            # Process extracted data into Graphiti
            ...
            if not processing_result.get("entities", []) and not processing_result.get("traits", []):
                logger.info(f"Skipping message {message.id} as no entities or traits were found")
                message.processed_in_graphiti = True  # Mark as processed
                message.is_stored_in_graphiti = False  # But we didn't store anything
                self.db.commit()
                return {
